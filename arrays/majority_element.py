...
def find_all_majority_elements(
    arr: List[int], threshold_fraction: float = 0.5
) -> List[int]:
    """
...
def benchmark_majority_algorithms():
    ...
    print(
        f"Majority element: {majority_element} (appears {test_array.count(majority_element)} times)"
    )
